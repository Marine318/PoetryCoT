...
model = Qwen2_5_VLForConditionalGeneration.from_pretrained(
    "/root/autodl-tmp/LLaMA-Factory/saves_cot/Qwen2.5-VL-7B_lora_cot_noregion",  # Path to your fine-tuned model
    torch_dtype=torch.bfloat16,
    device_map="auto",
    trust_remote_code=True  # Add this parameter to trust remote code
)
tokenizer = AutoTokenizer.from_pretrained("/root/autodl-tmp/LLaMA-Factory/saves_cot/Qwen2.5-VL-7B_lora_cot_noregion", padding_side='left', trust_remote_code=True)
processor = AutoProcessor.from_pretrained("/root/autodl-tmp/LLaMA-Factory/saves_cot/Qwen2.5-VL-7B_lora_cot_noregion", trust_remote_code=True)
...
def main(batch_size = 8):
    ...
    output_file = "/root/autodl-tmp/LLaMA-Factory/AnomalyCoT_output/Qwen2.5-VL-7B_lora_cot_noreg.jsonl"  # Path to output file for fine-tuned model
...
if __name__ == "__main__":
    main(1)  # Set batch size to 1 to avoid out of memory issues 