...
model = Qwen2VLForConditionalGeneration.from_pretrained(
    "/root/autodl-tmp/LLaMA-Factory/saves_cot/Qwen2-VL-7B-Instruct_lora_new",  # Path to local model
...
tokenizer = AutoTokenizer.from_pretrained(
    "/root/autodl-tmp/LLaMA-Factory/saves_cot/Qwen2-VL-7B-Instruct_lora_new", 
    padding_side='left', 
    trust_remote_code=True,
...
def batch_predict(batch_image_paths, batch_questions, model, processor, tokenizer):
    ...
    # Construct message list for Qwen2-VL format
    batch_messages = []
        ...
        messages = [
            ...
            },
            {
                ...
                "content": [
                    ...
                    {
                        "type": "text",
                        "text": question + " Please present your answer starting with The answer is (X = option). If the answer is \"no defect\", please provide a brief analysis; if not, immediately specify the anomaly region coordinates [[(?, ?), (?, ?)],...] and immediately followed by your analysis."
                    }
                ]
            }
        ]
...
def main(batch_size = 8):
    ...
    output_file = "/root/autodl-tmp/LLaMA-Factory/AnomalyCoT_output/Qwen2-VL-7B-Instruct_lora_new.jsonl"  # Path to output file
    os.makedirs(os.path.dirname(output_file), exist_ok=True)
    image_dir = "./COT_0423"  # Directory containing images
...
if __name__ == "__main__":
    main(2)  # Set batch size to 2, adjust based on GPU memory 